...
import { Plus, Search, Edit, Trash2, Package, X, UploadCloud, Clock, CheckCircle, AlertCircle, RotateCcw, Building, Package as PackageIcon } from "lucide-react";
    ...
    <div className="flex-1 min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 font-sans p-6 md:p-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          ...
          <button
            className="ml-2 px-4 py-2 rounded-lg bg-gradient-to-r from-yellow-300 via-emerald-400 to-cyan-400 text-slate-900 font-semibold flex items-center gap-2 shadow-lg shadow-emerald-500/20 hover:brightness-110 transition-all"
            onClick={openAdd}
          >
      ...
      {/* Filters */}
      <div className="rounded-2xl bg-slate-800/40 backdrop-blur-sm border border-slate-700/50 p-6 mb-6">
        <div className="flex flex-col md:flex-row gap-4 items-center">
          <div className="flex items-center gap-2">
            <Building className="w-4 h-4 text-emerald-400" />
            <label className="text-slate-300 text-sm whitespace-nowrap">Category:</label>
            <select
            ...
            >
              {categories.map(c => <option key={c} className="bg-slate-700 text-white">{c}</option>)}
            </select>
          </div>
          <div className="flex items-center gap-2">
            <PackageIcon className="w-4 h-4 text-cyan-400" />
            <label className="text-slate-300 text-sm whitespace-nowrap">Stock:</label>
            <select
              className="px-3 py-1.5 rounded-lg bg-slate-700/50 text-white border border-slate-600 focus:outline-none focus:ring-2 focus:ring-emerald-400/50 text-sm"
              value={stock}
              onChange={e => setStock(e.target.value)}
            >
              {stockStatuses.map(s => <option key={s} className="bg-slate-700 text-white">{s}</option>)}
            </select>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 max-w-full">
        {filtered.map(product => (
          <motion.div
            ...
            className="relative rounded-2xl bg-slate-800/40 backdrop-blur-sm border border-slate-700/50 p-6 flex flex-col gap-3 transition-all min-h-[240px] group"
            tabIndex={0}
          ...
          >
            ...
            <div className="w-12 h-12 flex items-center justify-center rounded-lg bg-gradient-to-br from-emerald-400/20 to-cyan-400/20">
              <Package className="w-7 h-7 text-slate-300" />
            </div>
            <div className="text-lg font-bold text-white mb-1">{product.name}</div>
            <div className="flex items-center gap-2 text-sm text-slate-300">
              <span className="bg-slate-600/30 px-2 py-0.5 rounded-full">{product.unit}</span>
      ...
      {editModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className="absolute inset-0 bg-black/60 backdrop-blur-[6px]" onClick={closeEdit} />
          <motion.form
          ...
          >
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  className="w-full px-3 py-2 rounded bg-slate-700/50 text-white border border-slate-600 focus:outline-none focus:ring-2 focus:ring-emerald-400/50"
                  name="name"
            ...
            <div className="flex justify-end gap-2 mt-8">
              <button type="button" className="px-4 py-2 rounded bg-slate-700/50 text-slate-200 hover:bg-slate-600/50" onClick={closeEdit}>Cancel</button>
              <button type="submit" className="px-4 py-2 rounded bg-gradient-to-r from-yellow-300 via-emerald-400 to-cyan-400 text-slate-900 font-semibold shadow-lg shadow-emerald-500/20 hover:brightness-110 transition-all">Save Changes</button>
            </div>
          </motion.form>
        </div>
      )}
      ...
      {deleteModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className="absolute inset-0 bg-black/60 backdrop-blur-[6px]" onClick={closeDelete} />
          <motion.div
            initial={{ scale: 0.95, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            exit={{ scale: 0.95, opacity: 0 }}
          ...
          >
            ...
            <div className="flex justify-end gap-2">
              <button type="button" className="px-4 py-2 rounded bg-slate-700/50 text-slate-200 hover:bg-slate-600/50" onClick={closeDelete}>Cancel</button>
              <button type="button" className="px-4 py-2 rounded bg-gradient-to-r from-red-400 to-orange-400 text-slate-900 font-semibold shadow-lg shadow-red-500/20 hover:brightness-110 transition-all" onClick={confirmDelete}>Delete</button>
            </div>
          </motion.div>
        </div>
      )}
      ...
      {addModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className="absolute inset-0 bg-black/60 backdrop-blur-[6px]" onClick={closeAdd} />
          <motion.form
            initial={{ scale: 0.95, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            exit={{ scale: 0.95, opacity: 0 }}
          ...
          >
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-slate-300 mb-1">Price per unit</label>
                <input
                  type="number"
                  className="w-full px-3 py-2 rounded bg-slate-700/50 text-white border border-slate-600 focus:outline-none focus:ring-2 focus:ring-emerald-400/50"
                  name="price"
              ...
              <div>
                <label className="block text-slate-300 mb-1">Product Icon (optional)</label>
                <label
                  htmlFor="icon-upload"
                  className="flex flex-col items-center justify-center w-full h-28 border-2 border-dashed border-emerald-400/40 rounded-lg cursor-pointer bg-slate-700/50 hover:border-emerald-400/60 transition group focus-within:border-emerald-400/60"
                  tabIndex={0}
                >
                  {addForm.icon ? (
                    <>
                    ...
                    <>
                      <UploadCloud className="w-8 h-8 text-cyan-400 mb-1" />
                      <span className="text-xs text-slate-400 group-hover:text-cyan-400">Drag & drop or click to upload</span>
                    </>
                  )}
                  <input
                    id="icon-upload"
            ...
            <div className="flex justify-end gap-2 mt-8">
              <button type="button" className="px-4 py-2 rounded bg-slate-700/50 text-slate-200 hover:bg-slate-600/50" onClick={closeAdd}>Cancel</button>
              <button type="submit" className="px-4 py-2 rounded bg-gradient-to-r from-yellow-300 via-emerald-400 to-cyan-400 text-slate-900 font-semibold shadow-lg shadow-emerald-500/20 hover:brightness-110 transition-all">Add Product</button>
            </div>
          </motion.form>
        </div>
      )}
